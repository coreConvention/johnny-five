...
> **There is no published image** — johnny-five is built locally. The first build
> installs `build-essential` and pre-downloads the `all-MiniLM-L6-v2` embedding
> model (~80–100 MB) into the image, so it can take a few minutes; subsequent
> builds reuse the Docker layer cache, and the running container never re-downloads
> the model.
...
### 2. Start the server (SSE — recommended)

```bash
docker volume create johnny-five-data   # one-time: compose declares the volume `external`
...
johnny-five is meant to be your **single** memory store. Claude Code also ships a
native auto-memory feature; leaving it on splits memories across two stores. Merge
this top-level key into `~/.claude/settings.json` (merge — don't overwrite the file):
...
This stops Claude Code's auto-memory from reading or writing its own store, so all
persistent memory flows through the `memory_*` tools. The `/memory` command still
works; only the automatic capture path is disabled. (The `/integrate` command can
set this for you.)
...
### Claude Code client settings

Separate from the server-side `MEMORY_*` variables, one **client-side** Claude Code
setting matters for keeping johnny-five as the single source of truth: set
`"autoMemoryEnabled": false` in `~/.claude/settings.json` to disable Claude Code's
native auto-memory (see [Quick Start step 4](#4-disable-claude-codes-built-in-memory-recommended)).
Otherwise memories split across two stores.
...
> **Don't want to do this by hand?** Run [`/integrate`](.claude/commands/integrate.md)
> from a clone — it performs every step in this section (CLAUDE.md discipline snippet,
> hooks, MCP wiring, optional per-project setup, and disabling Claude's native memory)
> idempotently and then runs a verification suite. The manual steps below are the
> by-hand alternative.

Johnny-five works best when Claude knows to use it automatically. The `setup/` directory contains ready-to-use snippets.

**How far you adopt is a choice of tier** — full walkthrough and ready-to-copy hook
configs in **[docs/INTEGRATION.md](docs/INTEGRATION.md)**:

| Tier | What it adds | When it's enough |
|---|---|---|
| **1 — Advisory** | Prompt hooks that *nudge* recall at session start and a store-check before stop | Trying it out; single machine |
| **2 — Compaction survival** | Command-type `SessionStart` + `PreCompact` hooks that mechanically recall and persist session-state across context compaction | Long sessions that compact |
...
> **How it works:** The Tier-1 `SessionStart` hook *prompts* Claude to call `memory_recall`; the enforced path (Tier 2+) instead uses a command-type `session-start-recall.sh` that calls it mechanically. The Tier-1 `Stop` hook prompts a store-check — but note the enforced snippet **omits** a prompt-type `Stop` hook (prompt-type Stop hooks can't reach MCP tools, since the server is shut down at stop time) and uses a command-type `Stop` enforcer instead (see [docs/INTEGRATION.md](docs/INTEGRATION.md)). All hooks are silent no-ops if the johnny-five server isn't connected.
