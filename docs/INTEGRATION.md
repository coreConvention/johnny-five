...
- [The tiers of integration](#the-tiers-of-integration)
...
- [Tier 4 — enforced search-first discipline](#tier-4--enforced-search-first-discipline)
...
| **4. Enforced search-first** | Search-first is *mandatory*, not advisory: relevant lessons are injected before risky tool use, and the turn can't end on a heavy edit without a store or on a correction without a search. | `PreToolUse` pre-search (`memory-context-inject.sh`) + `PostToolUse` counter (`memory-discipline-track.js`) + a **command-type `Stop` enforcer** (`memory-discipline-enforce.js`) that blocks Stop. | The discipline can no longer be skipped silently — the model is forced to store the lesson or explicitly bypass. |

You do **not** need to do all four at once. Tier 1 is enough to start seeing value. Tier 2 is recommended once you've hit one painful compaction. Tier 3 once you've hit "you already told me this" twice. Tier 4 when you want the search-first loop enforced rather than trusted.
...
### A note on prompt-type vs command-type Stop hooks

The `setup/hooks.json.snippet` (Tier 1) ships a **prompt-type** `Stop` hook that asks Claude to reflect before ending. A prompt-type Stop hook **cannot reach MCP tools** — MCP servers are shut down at Stop time — so a prompt like "now call `memory_store`" would fail. That's why the enforced snippet omits the *prompt-type* Stop hook.

It does, however, ship a **command-type** Stop hook ([Tier 4](#tier-4--enforced-search-first-discipline) below). A command-type Stop hook is just a process that reads a local state file and returns a decision — it never calls MCP, so the shutdown-at-Stop limitation doesn't apply. That's the supported way to enforce discipline at Stop.

---

## Tier 4 — enforced search-first discipline

### The problem Tier 3 doesn't solve

Tier 3 *searches* on a correction and *advises* after repeat failures, but nothing stops the model from skipping a search before a risky edit, or ending a turn where it made a dozen edits and stored nothing, or ending right after a correction without ever checking whether the answer was already in johnny-five. The discipline is still trusted, not enforced.

### The Tier 4 fix

Three hooks turn the search-first rule from advisory into mandatory:

- `memory-context-inject.sh` (**PreToolUse**) — before an `Edit`/`Write` on a risky code path, or a curated risky `Bash` command, it runs `memory_search` (top-2, `token_budget=400`, score ≥ 0.55 gate) and injects the hits as `additionalContext`, so prior lessons appear *before* the model acts. A 5-minute per-key cache prevents re-firing on consecutive edits. **The path glob and command patterns in this script are generic examples — tune them to your codebase's footguns.**
- `memory-discipline-track.js` (**PostToolUse**) — counts edits / stores / searches for the turn into `~/.claude/hooks/state/memory-discipline-<session>.json`. Never blocks; it only feeds the enforcer.
- `memory-discipline-enforce.js` (**command-type Stop**) — reads that state file and **blocks Stop** when either: (Gate 1) the turn had ≥ 3 edits and 0 `memory_store` calls, or (Gate 2) a correction fired this turn (`user-prompt-correction.sh` set `correction_seen`) and no `memory_search` has happened since. The block message tells the model exactly what to do.
...
1. **Copy the scripts**:
   ...
   chmod +x ~/.claude/hooks/memory-context-inject.sh
   ```
2. **Extend** `~/.claude/settings.json` with the `PreToolUse` and `Stop` entries and the **added** `PostToolUse` `memory-discipline-track.js` from [`setup/hooks.json.enforced.snippet`](../setup/hooks.json.enforced.snippet). If you already have `PostToolUse` hooks, **add** the tracker to the array rather than replacing it.
3. **Customize** `memory-context-inject.sh` — its trigger glob (`/(src|lib|...)/...`) and Bash command patterns are placeholders. Point them at the files and commands in *your* repo where a forgotten convention actually bites.
4. **Verify** the Stop enforcer with a synthetic state file:
   ```bash
   # simulate a heavy turn with no store, then attempt Stop
   mkdir -p ~/.claude/hooks/state
   echo '{"edits":5,"stores":0,"searches":0}' > ~/.claude/hooks/state/memory-discipline-test.json
   echo '{"session_id":"test"}' | node ~/.claude/hooks/memory-discipline-enforce.js
   # expect: {"decision":"block","reason":"Memory-discipline check: this turn made 5 file edits ..."}
   ```

### Bypassing the Stop block

The enforcer is meant to be overridable so it never traps you:

- **One-shot bypass** (this turn only): write `{"stop_overrides": true}` (merge with existing state) into `~/.claude/hooks/state/memory-discipline-<session>.json`, then re-attempt Stop. The block message tells the model how.
- **Disable for the whole session**: write `{"disabled": true}` into the same file.

Use bypass sparingly — every bypass is a turn where the lesson-capture trail goes silent.
...
Scoped to `project_dir=/projects/your-app`. 12 memories loaded.
